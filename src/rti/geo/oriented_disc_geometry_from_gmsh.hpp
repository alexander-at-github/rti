...
namespace rti { namespace geo {
  ...
  class oriented_disc_geometry_from_gmsh : public rti::geo::absc_geometry_from_gmsh<float> {
  public:
    oriented_disc_geometry_from_gmsh(RTCDevice& pDevice, rti::io::gmsh_reader& pGmshReader) :
  ...
  private:
    ...
    void init_this(RTCDevice& pDevice, rti::io::gmsh_reader& pGmshReader) {
      ...
      for (size_t idx = 0; idx < this->mNumTriangles; ++idx) {
        ...
        // Set normals
        rti::util::triple<float> normal = this->compute_triangle_normal(triangle);
